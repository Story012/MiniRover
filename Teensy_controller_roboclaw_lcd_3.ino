  ...
  for (int joystick_index = 0; joystick_index < COUNT_JOYSTICKS; joystick_index++) {
    if (joysticks[joystick_index].available()) {
      ...
      if (show_changed_only) {
        ...
        for (uint8_t i = 0; i<2; i++, axis_mask >>= 1) {
          if (axis_mask & 1) {
            Serial.printf(" %d:%d", i, joysticks[joystick_index].getAxis(i)); 
                     
            Serial2.write(Leftmotor(joysticks[joystick_index].getAxis(0),joysticks[joystick_index].getAxis(1)));
            Serial2.write(Rightmotor(joysticks[joystick_index].getAxis(0),joysticks[joystick_index].getAxis(1)));  
            lcd.clear();
            lcd.setCursor(0, 0);
            lcd.print("Left Motor:");
            lcd.print(Leftmotor(joysticks[joystick_index].getAxis(0),joysticks[joystick_index].getAxis(1))-64);
            lcd.setCursor(0, 1);
            lcd.print("Right Motor:");            
            lcd.print(Rightmotor(joysticks[joystick_index].getAxis(0),joysticks[joystick_index].getAxis(1))-192);         
          }
        }
      }
      uint8_t ltv;
...
*/
int Leftmotor(int a, int b){
  int throttle = a - 127;
  int dir = b - 127;
  int v = ((throttle - dir)/5)*1.2 + 64;
  if(v < 25){v = 32;}
  if(v > 103){v = 96;}
  return v;
  }

int Rightmotor(int a, int b){
  int throttle = a - 127;
  int dir = b - 127;
  int v = ((throttle + dir)/5)*1.2 + 192;
  if(v < 153){v = 160;}
  if(v > 234){v = 224;}
  return v;
  }